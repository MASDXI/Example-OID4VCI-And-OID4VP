...
export class HolderService {
    ...
    async acceptCredentialOffer(address: string, credentialOfferId: string): Promise<any> {
        try {
            ...
            const decodedJsonObject = JSON.parse(decodedString) as OpenId4VciResolvedCredentialOffer;
            ...
            const [prettyClaims] = storedCredentials.map((credential: any) => {
                return this.holder.agent.sdJwtVc.fromCompact(credential.compactSdJwtVc).prettyClaims;
            });
