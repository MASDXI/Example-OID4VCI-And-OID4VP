...
export class Holder extends BaseAgent<ReturnType<typeof getOpenIdHolderModules>> {
    ...
    public static async build(port: number): Promise<Holder> {
        const holder = new Holder(port, 'OpenId4VcHolder ' + Math.random().toString());
    ...
    public async resolveProofRequest(proofRequest: string) {
        const resolvedProofRequest =
            await this.agent.modules.openId4VcHolder.resolveSiopAuthorizationRequest(proofRequest);
